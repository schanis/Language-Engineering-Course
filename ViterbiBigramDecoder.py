...
class ViterbiBigramDecoder(object):
    ...
    def init_b(self):
        ...
        for i in range(Key.NUMBER_OF_CHARS):
            cs = Key.neighbour[i]  ## we get a list of neighbours
        ...
        for observation in range (1, len(index)):
            for hidden_state in range(Key.NUMBER_OF_CHARS):
                ...
                for letter in range (Key.NUMBER_OF_CHARS):
                    likelihood_estimates[letter] = self.v[observation-1][letter] + self.a[letter][hidden_state] + self.b[hidden_state][index[observation]]

                self.v[observation][hidden_state] = max(likelihood_estimates)
                self.backptr[observation+1][hidden_state] = np.argmax(likelihood_estimates)
